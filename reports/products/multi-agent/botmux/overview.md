...
- **上线时间与重要节点**：
  ...
  - 2026-09-09，项目发布 `v3.21.0`：commit 时间 15:22:13Z → annotated tag 时间 15:38:22Z → npm publish 时间 15:46:42Z；本机 CLI 与 npm latest 均为 3.21.0。tag 说明新增普通群项目协作、CLI 限额后备用 bot 交接、按任务配置模型、MiniMax CLI 适配器等。[12][13]
  - 截至 2026-09-13，完整 Git 历史约 3,347 commits；npm `time` 历史记录 432 次发布，当前 `versions` 对象保留 430 个可安装版本，说明项目仍处于高频迭代期。[12][13]
...
| 会话资源治理 | v3.21.0 源码默认每个 bot 最多 30 个 live workers；超限后按 LRU 挂起最长空闲且非进行中的会话，进行中回合不被打断 | 2026-09-13 | [18] |
...
3. **首跑开放与生产收紧的张力**：v3.21.0 源码在未配置任何 allowlist 时进入 open 模式，`canOperate` 返回 true；这降低首跑门槛，但生产必须配置 `allowedUsers` 等边界。源码同时把 `p2pOpen` 视为显式限制态，只开私聊但未配 owner 时操作权 fail-closed。[18]
4. **配置文件不是密钥库**：官方源码注释明确 per-bot env 与 `bots.json` 中的值以明文进入进程环境，可被本地诊断工具看到；适合个人可信主机，不满足集中密钥管理需求。[18]
5. **运维面不小**：需要常驻主机、tmux、目标 CLI 登录态、飞书应用权限、端口/远程访问策略、版本升级和多 bot 配置；对个人开发者可接受，对企业共享部署需要专门治理。[8][9][11]
6. **高频迭代与适配漂移**：6 个月内 npm 记录 432 次发布、主版本从 1 到 3，说明能力扩张快，也意味着升级回归、配置迁移和 adapter 兼容需要管理。[12][13]
7. **跨 CLI 不等于同一上下文**：官方明确无法跨 CLI 无损热切；多 Agent 协作仍依赖结构化 handoff，不能假设 Claude Code 与 Codex 共享同一原生记忆。[3]
8. **信息暴露与提示注入面**：群聊文本、代码输出、卡片标题、路径和终端截图会跨系统传输；多 Agent 群中一条恶意消息可能被多个 bot 解释，必须配合权限、沙盒、输出审查和最小工作区。[3][7][8]
...
| 18 | https://github.com/deepcoldy/botmux/tree/v3.21.0/src | 官方 v3.21.0 源码（权限、环境与 worker 治理） | 2026-09-13 |
...
| 2026-09-13 | 补充 v3.21.0 源码与发布流水线复核：open operate 模式、p2pOpen fail-closed、`bots.json` 非密钥库、默认 30 live workers、commit/tag/npm 三段时间线，以及 npm 发布数/可安装版本数双口径。 |
